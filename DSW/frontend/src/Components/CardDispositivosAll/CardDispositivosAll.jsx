...
export default function CardDispositivosAll({tipo, patrimonio, modelo, status, data, sala, onClickDeletar, onClickEditar, onClickCard}) {
    
    const handleCardClick = (e) => {
        if (e.target.closest('.carddispall-actions-buttons')) return;
        onClickCard();
    };
    return (
        
                <div className="card-dispall" onClick={handleCardClick}>
                    <div className='titlediv'>
                        <p className="text-dispall-tipo">
                            <b>Tipo:</b> {tipo}
                        </p>
                        <p className="text-dispall-sala">
                        {sala}
                        </p>
                    </div>
                    <p className="text-dispall-patrimonio">
                        <b>Patrimônio:</b> {patrimonio} 
                    </p>
                    <p className="text-dispall-modelo">
                        <b>Marca/Modelo:</b> {modelo}
                    </p>
                    <p className="text-dispall-status">
                        <b>Status:</b> {status}
                    </p>
                    <p className="text-dispall-data">
                        <b>Data de Verificação:</b> {data}
                    </p>
                    <div className="carddispall-actions-buttons">
                        <Tooltip title='Editar'>
                            <button className="tam3424 blue" onClick={onClickEditar}>
                                <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <g clipPath="url(#clip0_314_2914)">
